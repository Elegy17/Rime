...
更新内容: 
         ...
         2. 添加进入剪切板后自动隐藏状态栏的功能
            （查看词条中的置顶功能会更新系统的剪切板，请将自动状态栏.lua放入对应位置）
...
--------------------
<b>用法二</b>
第①步 将 脚本文件解压放置 Android/kuaima/script 文件夹内,
默认脚本路径为Android/kuaima/script/自定义剪切板.lua
...
preset_keys:
  lua_script_cvv: {label: 剪切板, functional: false, send: function, command: "剪切板.lua", option: "default"}
...
local 字体文件 = tostring(service.getLuaDir("")).."/fonts/Candidate.ttf"
...
if 当前配色背景颜色 != nil then
  --背景颜色
  背景颜色 = "0xFFF8F8F8" .. BigInteger(tostring(当前配色背景颜色), 10).toString(16)
...
local function fresh()
  ...
  if isSearch then
    ...
    ids.title.setText(标题.."(全部"..#arrNo.."条)")
    正则式 = "(\n*[^\n]*)(\n*[^\n]*)(\n*[^\n]*)"
  end
  for i=1,#arrNo do
...
--  end
  ...
  menu.add("🚮全部清空").onMenuItemClick=function(ae)
    io.open(文件,"w"):write("[\n]"):close()
